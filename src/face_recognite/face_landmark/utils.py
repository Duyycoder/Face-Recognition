...
def ref2dImagePoints(shape):
    imagePoints = [
        [shape.part(30).x, shape.part(30).y], [shape.part(8).x, shape.part(8).y], [
            shape.part(36).x, shape.part(
                36,
            ).y,
        ], [shape.part(45).x, shape.part(45).y], [shape.part(48).x, shape.part(48).y], [shape.part(54).x, shape.part(54).y],
    ]
    return np.array(imagePoints, dtype=np.float64)


def get_cameraMatrix(fl, center):
    mat = [[fl, 1, center[0]],  [0, fl, center[1]],  [0, 0, 1]]
...
def get_line(face_landmark, image, type='eye', debug=False):
    ...
    if type == 'eye':
        ...
        nose_length = (
            face_landmark['nose_bridge'][-1][1] -
            face_landmark['nose_bridge'][0][1]
        )
        ...
        mid_pointX = (
            face_landmark['nose_bridge'][-1][0] +
            face_landmark['nose_bridge'][0][0]
        ) / 2
...
def get_points_on_chin(line, face_landmark, chin_type='chin'):
    ...
    for i in range(len(chin) - 1):
        ...
        flag, x, y = line_intersection(
            line, (chin_first_point, chin_second_point),
        )
        if flag:
...
def line_intersection(line1, line2):
    ...
    line1 = (
        [line1[start][0], line1[start][1]],
        [line1[end][0], line1[end][1]],
    )
...
def convert_106p_to_86p(pt106):
    ...
    pt86 = {
        ...
        ], 'left_eye': [], 'right_eye': [], 'top_lip': [], 'bottom_lip': [], 'chin': [],
    }
    for i in range(68):
        if i < 17:
